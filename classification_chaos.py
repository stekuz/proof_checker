...
class BatchNormalization:
    ...
    def initialize(self, shape):
        self.gamma = tf.Variable(tf.ones(shape), dtype='float32')
        self.beta = tf.Variable(tf.zeros(shape), dtype='float32')
...
class FeedForwardLayer:
    ...
    def compute_loss(self, Y_pred, Y_true, latent=0, mean=1):
        #loss = tf.reduce_sum(tf.math.sqrt(abs(Y_true - Y_pred))) / (Y_pred.shape[0] * Y_pred.shape[1])
        if latent == 0:
            loss = keras.losses.binary_crossentropy(Y_true, Y_pred)
        else:
...
class ChaosTransformLayer:
    def __init__(self, input_shape, hidden_size, output_size, index, learning_rate=0.001, total_input=10, dropout=0, skipping=1):
        ...
        for i in range(self.total_input):
            self.dense_input.append(FeedForwardLayer(input_size=input_shape[1] * 3, output_size=hidden_size, index='ssoriginp' + str(index) + '_' + str(i), learning_rate=learning_rate, dropout=dropout))
        ...
        #self.chaos_large = np.array([[11, 2], [5, 1]], dtype='int64')
        ...
        self.mat_1 = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype='int64')
        self.chaos_1 = []
        self.chaos_1_inv = []
    ...
    def backward(self, X_original, A2, dA2, double_matrix=0, new_X=0, output=0, middle=0, skip_X=None):
        ...
        dA0 = (dA2 * dX[0])[:, :mxlen] + (dA2 * dX[0])[:, mxlen:2 * mxlen] + (dA2 * dX[0])[:, 2 * mxlen:]
        for i in range(1, self.total_input):
            ...
            dA0 += (dA2 * dX[i])[:, :mxlen] + (dA2 * dX[i])[:, mxlen:2 * mxlen] + (dA2 * dX[i])[:, 2 * mxlen:]
        return dA0
...
def train_chaos_words_imdb():
    ...
    def training_loop(epoch):
        ...
        for i in range(batches_selected):
            ...
            Y_pred_chaos = [0]
            skip_X = [0]
            Y_orig = tf.concat([Y, 1 - Y], axis=0)
            X_1 = tf.concat([X, tf.tile(tf.expand_dims(Y, axis=1), [1, X.shape[1], 1])], axis=2)
            X_0 = tf.concat([X, tf.tile(tf.expand_dims(1 - Y, axis=1), [1, X.shape[1], 1])], axis=2)
            X = tf.concat([X_1, X_0], axis=0)
            Y = tf.concat([tf.ones_like(Y), tf.zeros_like(Y)], axis=0)
            Y_pred_chaos[0], skip_X[0] = layers[0].forward(X, new_X=1, middle=middle)
            skip_X_orig = tf.concat((tf.zeros((X.shape[0], X.shape[1], 1)), X[:, :, 1:]), axis=2)
            Y_pred_chaos_with_order = []
            Y_pred_chaos_with_order.append(tf.concat([tf.broadcast_to(sequence_order_padding, (Y_pred_chaos[-1].shape[0], Y_pred_chaos[-1].shape[1], 1)),
                                                      tf.reshape(Y_pred_chaos[-1], (Y_pred_chaos[-1].shape[0], Y_pred_chaos[-1].shape[1], 1)),
                                                      tf.tile(tf.expand_dims(Y_orig, axis=1), [1, X.shape[1], 1])], axis=2))
        ...
        if (epoch % 100 < 20 or epoch % 100 == 0 or loss < threshold_loss or accuracy > 0.98 or 1) and len(validation_x) > 0:
            ...
            Y = validation_y
            zeros = tf.zeros((X.shape[0], X.shape[1], 1))
            ones = tf.ones((X.shape[0], X.shape[1], 1))
            X_0 = tf.concat([X, zeros], axis=2)
            X_1 = tf.concat([X, ones], axis=2)
            #count predictions for zeros
            X = X_0
            Y_pred_chaos = [0]
            skip_X = [0]
            ...
            for j in range(1, len(layers) - 1):
                ...
                Y_pred_chaos_with_order.append(tf.concat([tf.broadcast_to(sequence_order_padding, (Y_pred_chaos[-1].shape[0], Y_pred_chaos[-1].shape[1], 1)),
                                                          tf.reshape(Y_pred_chaos[-1], (Y_pred_chaos[-1].shape[0], Y_pred_chaos[-1].shape[1], 1)),
                                                          zeros], axis=2))
            Y_pred_final_zeros, _ = layers[-1].forward(Y_pred_chaos_with_order[-1], new_X=1, output=1, skip_X=skip_X_orig, training=0)
            #count predictions for ones
            X = X_1
            Y_pred_chaos = [0]
            skip_X = [0]
            Y_pred_chaos[0], skip_X[0] = layers[0].forward(X, new_X=1, middle=middle, training=0)
            ...
            for j in range(1, len(layers) - 1):
                ...
                Y_pred_chaos_with_order.append(tf.concat([tf.broadcast_to(sequence_order_padding, (Y_pred_chaos[-1].shape[0], Y_pred_chaos[-1].shape[1], 1)),
                                                          tf.reshape(Y_pred_chaos[-1], (Y_pred_chaos[-1].shape[0], Y_pred_chaos[-1].shape[1], 1)),
                                                          ones], axis=2))
            Y_pred_final_ones, _ = layers[-1].forward(Y_pred_chaos_with_order[-1], new_X=1, output=1, skip_X=skip_X_orig, training=0)
            Y_pred_final = tf.cast(Y_pred_final_ones > Y_pred_final_zeros, dtype='float32')
            #print(Y_pred_final_ones[:20], Y_pred_final_zeros[:20])
            val_loss = dense_output.compute_loss(Y_pred_final, Y)
            val_accuracy = dense_output.compute_accuracy(Y_pred_final, Y)
            print(f'    epoch: {epoch}, validation loss: {val_loss}, validation accuracy: {val_accuracy}')
        if epoch % 100 == 0 or (loss < threshold_loss or accuracy > 0.98) and 0 or val_accuracy > 0.98 or 1:
